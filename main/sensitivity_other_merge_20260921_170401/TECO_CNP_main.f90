...
WRITE(*,*) "P_add_rate_kg_ha_yr = ", P_add_rate_kg_ha_yr
    ...
    IF (MCMC.eq.0) THEN ! START BIOGEOCHEMICAL CYCLE SIMULATION
        ...
        ! OUTPUTS
<<<<<<< /home/user/fangwei/FangxiuWan-TECO-CNP-Sv1.0-edff198/TECO-CNP Sv1.0 20250316/main/TECO_CNP_main.f90
...
||||||| /home/user/fangwei/TECO_sensitivity_server/baseline_main/TECO_CNP_main.f90
        CALL FilesForOutput ()   !Configuring the file path for model output
        CALL WriteFiles_noMCMC() !Customize the outputs based on research requirements
=======
        IF (teco_sensitivity_mode .EQ. 1) THEN
            ! Sensitivity runs write one compact row and deliberately skip
            ! the large standard output files.  This also prevents runs from
            ! overwriting the user's formal simulation results.
            CALL WriteSensitivitySummary()
        ELSE
            CALL FilesForOutput ()   !Configuring the file path for model output
            CALL WriteFiles_noMCMC() !Customize the outputs based on research requirements
        ENDIF
>>>>>>> /home/user/fangwei/TECO_sensitivity_server/patched_main/TECO_CNP_main.f90
    ELSE!START MCMC
        WRITE(*,*) 'STARTING MCMC'
        ! SET SEED
...
||||||| /home/user/fangwei/TECO_sensitivity_server/baseline_main/TECO_CNP_main.f90
    ...
    eS  = 0.05 
=======
    k_n = 0.5  * sens_alloc_kn
    ww = 0.36  * sens_alloc_ww  !origin: 0.8
    eL  = 0.35  * sens_alloc_el
    eS  = 0.05  * sens_alloc_es
>>>>>>> /home/user/fangwei/TECO_sensitivity_server/patched_main/TECO_CNP_main.f90
    eR  = 0.60
...
||||||| /home/user/fangwei/TECO_sensitivity_server/baseline_main/TECO_CNP_main.f90
        a_Re=0.12 ! wan assume a fixed allocation fraction for fecundity 12% of new carbon
=======
        a_Re=0.12*sens_alloc_repro ! sensitivity multiplier; default preserves 0.12
>>>>>>> /home/user/fangwei/TECO_sensitivity_server/patched_main/TECO_CNP_main.f90
    ENDIF
